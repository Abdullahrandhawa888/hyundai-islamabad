...
export function WhatsAppButton() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (goingDown && y > 80) {
        ...
        hideTimer.current = setTimeout(() => setHidden(false), 1200);
      } else if (goingUp) {
        setHidden(false);
        clearTimeout(hideTimer.current);
      }
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className={`fixed right-4 bottom-5 z-40 flex items-center gap-2 transition-all duration-300 ease-out hover:scale-105 ${
    ...
    >
      ...
      <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg md:h-14 md:w-14">
